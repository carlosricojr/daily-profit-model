...
class OptimizationValidator:
    ...
    def validate_optimization(
        ...
        logger.info(
            f"Testing optimization with {len(test_accounts)} accounts from {start_date} to {end_date}"
        )
        ...
        original_records = self._run_original_implementation(
        ...
        results["original"] = {
            ...
            "queries_executed": self._estimate_original_queries(
                test_accounts, test_days
            ),
        }

        # Get original results
        original_features = self._get_features_snapshot(
            test_accounts, start_date, end_date
        )
        ...
        optimized_records = self._run_optimized_implementation(
            test_accounts, start_date, end_date
        )
        ...
        optimized_features = self._get_features_snapshot(
            test_accounts, start_date, end_date
        )
        ...
        query_reduction = (
            (
                results["original"]["queries_executed"]
                - results["optimized"]["query_stats"]["bulk_queries"]
            )
            / results["original"]["queries_executed"]
            * 100
            if results["original"]["queries_executed"] > 0
            else 0
        )
    ...
    def _run_optimized_implementation(
        self, account_ids: List[str], start_date: date, end_date: date
    ) -> int:
        ...
        return self.optimized_engineer._process_batch_optimized(
    ...
    def _compare_features(
        self, original_df: pd.DataFrame, optimized_df: pd.DataFrame
    ) -> Dict[str, Any]:
                                    ...
                                    comparison["differences"].append(
                                        {
                                            ...
                                            "sample_original": float(orig_vals.iloc[0])
                                            if len(orig_vals) > 0
                                            else None,
                                            "sample_optimized": float(opt_vals.iloc[0])
                                            if len(opt_vals) > 0
                                            else None,
                                        }
                                    )
    ...
    def _log_validation_summary(self, results: Dict[str, Any]):
        ...
        logger.info(
            f"  - Execution time: {results['original']['execution_time']:.2f} seconds"
        )
        logger.info(f"  - Estimated queries: {results['original']['queries_executed']}")

        logger.info("\nOptimized implementation:")
        logger.info(
            f"  - Execution time: {results['optimized']['execution_time']:.2f} seconds"
        )
        logger.info(
            f"  - Bulk queries: {results['optimized']['query_stats']['bulk_queries']}"
        )
        logger.info(
            f"  - Query reduction: {results['performance_improvement']['query_reduction_pct']:.1f}%"
        )

        logger.info("\nIMPROVEMENT:")
        logger.info(
            f"  - Speedup: {results['performance_improvement']['speedup_factor']}x faster"
        )
        logger.info(
            f"  - Time saved: {results['performance_improvement']['time_saved_seconds']:.2f} seconds"
        )

        # Results comparison
        logger.info("\nRESULTS COMPARISON:")
        logger.info(
            f"Records created - Original: {results['original']['records_created']}, Optimized: {results['optimized']['records_created']}"
        )
        ...
        if results["comparison"]["all_values_match"]:
            ...
            logger.warning(
                f"✗ Found {len(results['comparison']['differences'])} differences:"
            )
            for diff in results["comparison"]["differences"][:5]:  # Show first 5
                logger.warning(
                    f"  - {diff['column']}: {diff['type']} - {diff.get('details', '')}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test feature engineering optimization"
    )
