    ...
    try:
        ...
        if args.validate:
            ...
            if "target_statistics" in validation:
                ...
                if stats.get('mean_pnl') is not None:
                    logger.info(f"  - Mean: ${stats['mean_pnl']:.2f}")
                if stats.get('std_pnl') is not None:
                    logger.info(f"  - Std Dev: ${stats['std_pnl']:.2f}")
                if stats.get('min_pnl') is not None:
                    logger.info(f"  - Min: ${stats['min_pnl']:.2f}")
                if stats.get('max_pnl') is not None:
                    logger.info(f"  - Max: ${stats['max_pnl']:.2f}")
                if stats.get('median_pnl') is not None:
