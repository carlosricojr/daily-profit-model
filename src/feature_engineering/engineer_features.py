...
# Core feature engineering constants and configuration
FEATURE_VERSION = "3.1.0"  # Integrated version with all optimizations


def get_optimal_config():
    cpu_count = mp.cpu_count()  # 32 on your system
    total_ram_mb = psutil.virtual_memory().total // (1024**2)  # ~125GB
...
class OptimizedFeatureEngineer:
    ...
    def _bulk_fetch_static_features(
        ...
        self.query_stats["total_queries"] += len(account_ids) * len(
            dates
    ...
    def _bulk_fetch_performance_data(
        ...
        self.query_stats["total_queries"] += (
            len(account_ids) * 60
        )  # Assuming ~60 queries for rolling windows
    ...
    def _bulk_fetch_trades_data(
        ...
        self.query_stats["total_queries"] += (
            len(account_ids) * 5
        )  # Assuming ~5 days window per account
    ...
    def _calculate_features_from_bulk_data(
        self, account_id: str, login: str, feature_date: date, bulk_data: Dict[str, Any]
...
class IntegratedProductionFeatureEngineer(OptimizedFeatureEngineer):
    ...
    def engineer_features_with_validation(
        self,
        ...
        logger.info(
            f"[INTEGRATED] Starting feature engineering from {start_date} to {end_date}"
        )
        logger.info(
    ...
    def _process_batches_with_validation(
        self,
        ...
        for i in range(0, len(work_items), batch_size):
            ...
            with self.query_monitor.track_query(
                "process_batch", f"Processing batch of {len(batch)} items"
            ):
                batch_records = self._process_batch_enhanced(
                    batch, start_date, end_date, validate_bias
                )
    ...
    def _process_batch_enhanced(
        ...
        bulk_data = self._bulk_fetch_all_data_monitored(
    ...
    def _bulk_fetch_all_data_monitored(
        self,
    ...
    def _calculate_and_validate_features(
        self,
        account_id: str,
        login: str,
        feature_date: date,
