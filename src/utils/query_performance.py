...
class QueryPerformanceMonitor:
    ...
    def _log_slow_query(self, query_info: Dict, query: str, params: Any):
        ...
        logger.warning(
            f"Slow query detected ({query_info['execution_time']:.2f}s): {query_info['type']}"
        )
    ...
    def save_performance_log(self):
        ...
        try:
            log_data = {
                ...
                "recent_queries": self.query_stats["query_log"][
                    -20:
                ],  # Last 20 queries
            }
...
class DatabaseConnectionPool:
    ...
    def execute_query(
        self, query: str, params: Any = None, query_type: str = "generic"
    ) -> List[Dict]:
...
if __name__ == "__main__":
    ...
    with monitor.track_query(
        "select_features", "SELECT * FROM features WHERE account_id = %s"
    ):
    ...
    with monitor.track_query(
        "select_features", "SELECT * FROM features WHERE date = %s"
    ):
