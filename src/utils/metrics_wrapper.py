...
from .metrics import get_prometheus_metrics, get_aggregated_metrics
...
class MetricsCollector:
    ...
    def record_timing(
        ...
        key = self._make_key(name, labels)
        self._timings[key].append({"timestamp": time.time(), "duration": duration})
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {"counters": {}, "gauges": dict(self._gauges), "timings": {}}
        ...
        if self.metrics_collector:
            ...
            if exc_type is not None:
                self.metrics_collector.increment_counter(
                    f"{self.metric_name}_error", 1, self.labels
                )
            else:
                self.metrics_collector.increment_counter(
                    f"{self.metric_name}_success", 1, self.labels
                )
