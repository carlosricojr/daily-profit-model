...
class DataLineageTracker:
    ...
    def _build_lineage_graph(self):
        ...
        nodes = [
            ...
            LineageNode(
                "raw_plans", LineageNodeType.SOURCE_TABLE, "Raw Plans", "raw_plans_data"
            ),
            LineageNode(
                "raw_regimes",
                LineageNodeType.SOURCE_TABLE,
            ...
            LineageNode(
                "stg_snapshots",
                LineageNodeType.STAGING_TABLE,
                "Staging Snapshots",
                "stg_accounts_daily_snapshots",
            ),
            # Transformations
            LineageNode(
                "snapshot_creation", LineageNodeType.TRANSFORMATION, "Snapshot Creation"
            ),
            LineageNode(
                "data_validation", LineageNodeType.VALIDATION, "Data Validation"
            ),
            LineageNode(
                "feature_engineering",
    ...
    def _generate_lineage_id(
        self, source: str, target: str, transformation: str, date: date
    ) -> str:
        """Generate unique lineage ID."""
        components = (
            f"{source}_{target}_{transformation}_{date}_{datetime.now().timestamp()}"
        )
    ...
    def get_upstream_lineage(
        self, table_name: str, levels: int = 3
    ) -> Dict[str, List[str]]:
        """
    ...
    def get_downstream_impact(
        self, table_name: str, processing_date: Optional[date] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_data_freshness(self, table_name: str) -> Dict[str, Any]:
        ...
        if result and result[0]["latest_date"]:
            freshness = result[0]
            age_hours = (
                datetime.now() - freshness["latest_update"]
            ).total_seconds() / 3600
    ...
    def visualize_lineage(self, table_name: str, output_file: Optional[str] = None):
        ...
        try:
            ...
            if table_name in self.graph:
                ...
                staging_patch = mpatches.Patch(
                    color="lightgreen", label="Staging Tables"
                )
                feature_patch = mpatches.Patch(
                    color="lightcoral", label="Feature Tables"
                )
                transform_patch = mpatches.Patch(
                    color="lightyellow", label="Transformations"
                )
    ...
    def generate_lineage_report(
        self, start_date: date, end_date: date, output_file: Optional[str] = None
    ) -> str:
        ...
        report_lines.extend(["TABLE FRESHNESS", "-" * 40])
