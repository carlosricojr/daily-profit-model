...
class AlertManager:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "kafka": {
                "enabled": KAFKA_AVAILABLE
                and bool(os.getenv("KAFKA_BOOTSTRAP_SERVERS")),
                "bootstrap_servers": os.getenv(
                    "KAFKA_BOOTSTRAP_SERVERS", "localhost:9092"
                ),
                "topic": os.getenv("KAFKA_ALERT_TOPIC", "data-quality-alerts"),
            },
            "database": {"enabled": True, "table_name": "data_quality_alerts"},
            "severity_thresholds": {
    ...
    def _format_alert_text(self, alert: DataQualityAlert) -> str:
        ...
        if alert.recommended_action:
            lines.extend(["", "Recommended Action:", f"{alert.recommended_action}"])
    ...
    def check_and_alert_data_quality(
        self, validation_results: List[Any], anomaly_results: List[Any]
    ...
    def get_alert_history(
        self, start_date: date, end_date: date, severity: Optional[AlertSeverity] = None
    ) -> List[Dict]:
