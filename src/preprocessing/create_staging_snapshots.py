...
class StagingSnapshotsCreator:
    ...
    def _create_snapshots_for_date(self, snapshot_date: date) -> int:
        ...
        INSERT INTO prop_trading_model.stg_accounts_daily_snapshots (
            ...
            top_symbol_concentration, herfindahl_index, symbol_diversification_index,
            forex_pairs_ratio, metals_ratio, index_ratio, crypto_ratio, energy_ratio,
            -- Trade aggregations
            trades_last_1d, trades_last_7d, trades_last_30d,
            volume_usd_last_1d, volume_usd_last_7d, volume_usd_last_30d,
            profit_last_1d, profit_last_7d, profit_last_30d,
            unique_symbols_last_7d, unique_symbols_last_30d,
            -- Trade timing patterns
            trades_morning_ratio, trades_afternoon_ratio,
            trades_evening_ratio, trades_weekend_ratio, avg_trade_hour_of_day,
            -- Rolling metrics
            sharpe_ratio_5d, sharpe_ratio_10d, sharpe_ratio_20d,
            profit_volatility_5d, profit_volatility_10d, profit_volatility_20d,
            win_rate_5d, win_rate_10d, win_rate_20d,
            balance_change_5d, balance_change_10d, balance_change_20d,
            equity_high_water_mark, days_since_equity_high,
            -- Performance metrics from raw_metrics_daily
            daily_sharpe, daily_sortino,
            profit_factor, success_rate,
        ...
        SELECT 
            ...
            COALESCE(ta.volume_usd_last_1d, 0) as volume_usd_last_1d,
            COALESCE(ta.volume_usd_last_7d, 0) as volume_usd_last_7d,
            COALESCE(ta.volume_usd_last_30d, 0) as volume_usd_last_30d,
            COALESCE(ta.profit_last_1d, 0) as profit_last_1d,
            COALESCE(ta.profit_last_7d, 0) as profit_last_7d,
            COALESCE(ta.profit_last_30d, 0) as profit_last_30d,
            ...
            COALESCE(am.profit_factor, 0) as profit_factor,
            LEAST(GREATEST(COALESCE(am.success_rate, 0), 0), 1)::numeric(5,4) as success_rate,
            COALESCE(am.mean_profit, 0) as mean_profit,
            COALESCE(am.median_profit, 0) as median_profit,
            COALESCE(am.mean_drawdown, 0) as mean_drawdown,
            COALESCE(am.median_drawdown, 0) as median_drawdown,
            COALESCE(am.mean_duration, 0) as mean_duration,
            COALESCE(am.median_duration, 0) as median_duration,
            COALESCE(am.total_lots, 0) as total_lots,
            COALESCE(am.total_volume, 0) as total_volume
        FROM account_metrics am
