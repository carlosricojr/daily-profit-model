...
class LateDataHandler:
    ...
    def detect_late_arriving_data(
        self, check_date: date, lookback_hours: int = 48
    ) -> List[LateDataEvent]:
        """
    ...
    def _check_late_accounts(
        self, check_date: date, lookback_hours: int
    ) -> List[LateDataEvent]:
        ...
        for account in late_accounts:
            delay_hours = (
                account["first_seen"] - watermark["watermark_timestamp"]
            ).total_seconds() / 3600
            ...
            event_type = (
                LateDataType.NEW_ACCOUNT if exists == 0 else LateDataType.CORRECTED_DATA
            )
    ...
    def _check_late_metrics(
        ...
        for account_id, metrics in account_metrics.items():
            delay_hours = max(
                (
                    m["ingestion_timestamp"] - watermark["watermark_timestamp"]
                ).total_seconds()
                / 3600
                for m in metrics
            )
    ...
    def _check_late_trades(
        ...
        if result and result[0]["late_trade_count"] > 0:
            stats = result[0]
            delay_hours = (
                stats["latest"] - watermark["watermark_timestamp"]
            ).total_seconds() / 3600
            ...
            events.append(
                LateDataEvent(
                    ...
                    details={"affected_accounts": stats["affected_accounts"]},
                )
            )

        return events

    def process_late_data(
        self, events: List[LateDataEvent], reprocess_features: bool = True
    ) -> Dict[str, int]:
        """
    ...
    def _update_staging_snapshots(
        self, affected_date: date, events: List[LateDataEvent]
    ) -> int:
        ...
        for account_id in affected_accounts:
            ...
            logger.info(
                f"Recreating snapshot for account {account_id} on {affected_date}"
            )
            updated_count += 1

        return updated_count

    def _reprocess_features(
        self, affected_date: date, events: List[LateDataEvent]
    ) -> int:
        ...
        logger.info(
            f"Reprocessing features for {len(affected_accounts)} accounts on {affected_date}"
        )
    ...
    def _get_watermark(self, table_name: str, processing_date: date) -> Optional[Dict]:
        ...
        results = self.db_manager.model_db.execute_query(
            query, (table_name, processing_date)
        )
    ...
    def create_watermark(
        self, table_name: str, processing_date: date, watermark_timestamp: datetime
    ):
    ...
    def get_late_data_summary(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        if summary["total_events"] > 0:
            ...
            if delay_result and delay_result[0]["weighted_avg_delay"]:
                summary["avg_delay_hours"] = float(
                    delay_result[0]["weighted_avg_delay"]
                )
