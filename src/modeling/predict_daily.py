...
class ShadowDeploymentManager:
    ...
    def create_shadow_deployment(
        ...
        try:
            ...
            logger.info(
                f"  Traffic percentage: {deployment_config.get('traffic_percentage', 100)}%"
            )
    ...
    def update_shadow_performance(
        self, deployment_id: str, performance_metrics: Dict[str, Any]
    ):
        ...
        try:
            self.db_manager.model_db.execute_command(
                update_query, (json.dumps(performance_metrics), deployment_id)
            )
        except Exception as e:
            logger.warning(f"Failed to update shadow performance: {e}")

    def compare_shadow_vs_production(
        self, deployment_id: str, evaluation_period_days: int = 7
    ) -> Dict[str, Any]:
        ...
        deployment = self.db_manager.model_db.execute_query(
            deployment_query, (deployment_id,)
        )
        if not deployment:
        ...
        try:
            ...
            # Process results
            metrics_by_model = {
                result["model_version"]: result for result in comparison_results
            }
            ...
            if shadow_metrics and production_metrics:
                if production_metrics.get("avg_mae") and shadow_metrics.get("avg_mae"):
                    mae_improvement = (
                        ...
                        / production_metrics["avg_mae"]
                        * 100
                    )
                    comparison["mae_improvement_pct"] = mae_improvement

                if production_metrics.get("direction_accuracy") and shadow_metrics.get(
                    "direction_accuracy"
                ):
                    accuracy_improvement = (
                        shadow_metrics["direction_accuracy"]
                        - production_metrics["direction_accuracy"]
                    ) * 100
                    comparison["accuracy_improvement_pct"] = accuracy_improvement

                # Determine recommendation
                mae_better = shadow_metrics.get(
                    "avg_mae", float("inf")
                ) < production_metrics.get("avg_mae", float("inf"))
                accuracy_better = shadow_metrics.get(
                    "direction_accuracy", 0
                ) > production_metrics.get("direction_accuracy", 0)
...
class RealTimeDriftDetector:
    ...
    def load_reference_distributions(self, model_version: str):
        ...
        if reference_data_path.exists():
            try:
                self.reference_distributions[model_version] = joblib.load(
                    reference_data_path
                )
    ...
    def detect_real_time_drift(
        ...
        if model_version in self.reference_distributions:
            ...
            drift_results["feature_drift"] = {
                ...
                "drifted_features": [
                    f
                    for f, score in feature_drift_scores.items()
    ...
    def _calculate_feature_drift(
        self, reference_series: pd.Series, current_series: pd.Series
    ) -> float:
    ...
    def _detect_prediction_drift(
        self, current_predictions: np.ndarray, model_version: str
    ) -> Dict[str, Any]:
        ...
        try:
            historical_df = self.db_manager.model_db.execute_query_df(
                query, (model_version,)
            )
            ...
            drift_detected = (
                mean_shift > self.drift_thresholds["prediction_drift"]
                or std_ratio > 2.0
                or std_ratio < 0.5
            )
...
class AdvancedDailyPredictor:
    ...
    def _load_model(self):
        ...
        # Use the correct table name from schema
    ...
    def _load_shadow_model(self, model_version: str) -> Dict[str, Any]:
        ...
        result = self.db_manager.model_db.execute_query(
            query, (model_version, model_version, model_version)
        )
    ...
    def _process_prediction_batch(
        ...
        if enable_drift_detection:
            try:
                ...
                results_df["drift_detected"] = drift_results.get(
                    "overall_drift_detected", False
                )
                results_df["feature_drift_count"] = len(
                    drift_results.get("feature_drift", {}).get("drifted_features", [])
                )
                results_df["prediction_drift_detected"] = drift_results.get(
                    "prediction_drift", {}
        ...
        if enable_shadow_deployment and self.shadow_models:
            ...
            for shadow_version, shadow_data in shadow_predictions.items():
                results_df[f"shadow_{shadow_version}_prediction"] = shadow_data[
                    "predictions"
                ]
                results_df[f"shadow_{shadow_version}_confidence"] = shadow_data.get(
                    "confidence", 0.5
                )

        # Calculate enhanced confidence and risk scores
        results_df = self._enhance_predictions(
            results_df, batch_features, drift_results
        )
    ...
    def _generate_shadow_predictions(
        self, X: pd.DataFrame, batch_features: pd.DataFrame
        ...
        for shadow_version, shadow_data in self.shadow_models.items():
            try:
                ...
                if (
                    len(common_features) < len(shadow_features) * 0.8
                ):  # Need at least 80% features
                    logger.warning(
                        f"Shadow model {shadow_version} has incompatible features"
                    )
                    continue
                ...
                X_shadow_scaled[numerical_cols] = shadow_data["scaler"].transform(
                    X_shadow_prepared[numerical_cols]
                )
                ...
                logger.debug(
                    f"Generated {len(X_shadow)} shadow predictions for {shadow_version}"
                )

            except Exception as e:
                logger.warning(
                    f"Failed to generate shadow predictions for {shadow_version}: {e}"
                )
    ...
    def _get_features_for_prediction(self, feature_date: date) -> pd.DataFrame:
        ...
        if not features_df.empty:
            ...
            if "active_trading_days_count" in features_df.columns:
                sufficient_history = (
                    features_df["active_trading_days_count"] >= min_trading_days
                )
                if not sufficient_history.all():
                    logger.info(
                        f"Filtered out {(~sufficient_history).sum()} accounts with < {min_trading_days} trading days"
                    )
            ...
            for col in numerical_cols:
                if col in features_df.columns:
                    ...
                    extreme_mask = (features_df[col] > q99 * 10) | (
                        features_df[col] < q1 * 10
                    )
                    if extreme_mask.any():
                        logger.warning(
                            f"Found {extreme_mask.sum()} extreme values in {col}"
                        )
    ...
    def _enhance_predictions(
        ...
        if drift_results.get("overall_drift_detected", False):
            confidence_penalty = min(
                20,
                len(drift_results.get("feature_drift", {}).get("drifted_features", []))
                * 3,
            )
        ...
        if not batch_features.empty:
            completeness = (
                1
                - batch_features[self.feature_columns].isnull().sum(axis=1)
            ...
            for _, row in predictions_df.iterrows():
                ...
                for col in row.index:
                    if col.startswith("shadow_") and col.endswith("_prediction"):
                        shadow_version = col.replace("shadow_", "").replace(
                            "_prediction", ""
                        )
                        if not pd.isna(row[col]):
                            shadow_preds[shadow_version] = {
                                ...
                                "confidence": float(
                                    row.get(f"shadow_{shadow_version}_confidence", 0.5)
                                ),
                            }
                ...
                record = {
                    ...
                    "batch_processing_time_ms": float(
                        row.get("batch_processing_time_ms", 0)
                    ),
                }
    ...
    def _log_advanced_batch_summary(
        self, predictions_df: pd.DataFrame, batch_id: str, total_time_ms: float
    ):
        ...
        logger.info(
            f"  Avg time per prediction: {total_time_ms / len(predictions_df):.2f}ms"
        )
        logger.info(
            f"  Mean predicted PnL: ${predictions_df['predicted_net_profit'].mean():.2f}"
        )
        logger.info(
            f"  Predicted profits: {(predictions_df['predicted_net_profit'] > 0).sum()}"
        )
        logger.info(
            f"  Predicted losses: {(predictions_df['predicted_net_profit'] < 0).sum()}"
        )
        logger.info(
            f"  Average confidence: {predictions_df['prediction_confidence'].mean():.1f}%"
        )

        if "drift_detected" in predictions_df.columns:
            drift_count = predictions_df["drift_detected"].sum()
            logger.info(
                f"  Drift detected: {drift_count} accounts ({drift_count / len(predictions_df) * 100:.1f}%)"
            )

        if "is_high_risk" in predictions_df.columns:
            high_risk_count = predictions_df["is_high_risk"].sum()
            logger.info(
                f"  High risk accounts: {high_risk_count} ({high_risk_count / len(predictions_df) * 100:.1f}%)"
            )
            ...
            try:
                ...
                batch_metrics = {
                    ...
                    "avg_confidence": float(
                        predictions_df["prediction_confidence"].mean()
                    ),
                    "high_risk_rate": float(
                        (predictions_df.get("is_high_risk", False)).mean()
                    ),
                    "drift_detection_rate": float(
                        (predictions_df.get("drift_detected", False)).mean()
                    ),
                }
                ...
                for shadow_version in self.shadow_models.keys():
                    ...
                    if shadow_col in predictions_df.columns:
                        ...
                        if len(shadow_predictions) > 0:
                            batch_metrics[f"{shadow_version}_predictions"] = len(
                                shadow_predictions
                            )
                            batch_metrics[f"{shadow_version}_avg_prediction"] = float(
                                shadow_predictions.mean()
                            )
    ...
    def create_shadow_deployment(
        ...
        deployment_id = self.shadow_manager.create_shadow_deployment(
            shadow_model_version, self.model_version, deployment_config
        )
    ...
    def evaluate_shadow_deployments(
        self, evaluation_period_days: int = 7
    ) -> Dict[str, Any]:
        ...
        for deployment in active_deployments:
            ...
            try:
                ...
                if "mae_improvement_pct" in comparison:
                    logger.info(
                        f"  MAE improvement: {comparison['mae_improvement_pct']:.1f}%"
                    )
                if "accuracy_improvement_pct" in comparison:
                    logger.info(
                        f"  Accuracy improvement: {comparison['accuracy_improvement_pct']:.1f}%"
                    )
                ...
                logger.error(
                    f"Failed to evaluate shadow deployment {deployment_id}: {e}"
                )
...
def main():
    ...
    parser.add_argument(
        "--no-save", action="store_true", help="Do not save predictions"
    )
    parser.add_argument(
        "--disable-shadow", action="store_true", help="Disable shadow deployment"
    )
    parser.add_argument(
        "--disable-drift-detection", action="store_true", help="Disable drift detection"
    )
    parser.add_argument(
        "--batch-size", type=int, default=1000, help="Batch size for processing"
    )
    parser.add_argument(
        "--create-shadow", help="Create shadow deployment with specified model version"
    )
    parser.add_argument(
        "--shadow-traffic",
        type=float,
    ...
    try:
        if args.create_shadow:
            ...
            deployment_id = predictor.create_shadow_deployment(
                args.create_shadow, args.shadow_traffic
            )
            logger.info(f"Created shadow deployment: {deployment_id}")
            ...
            for deployment_id, result in results.items():
                ...
                if "error" in result:
                    ...
                    logger.info(
                        f"  Shadow Model: {result.get('shadow_model', 'Unknown')}"
                    )
                    logger.info(
                        f"  Production Model: {result.get('production_model', 'Unknown')}"
                    )
                    ...
                    if "accuracy_improvement_pct" in result:
                        logger.info(
                            f"  Accuracy Improvement: {result['accuracy_improvement_pct']:.1f}%"
                        )
            ...
            if not predictions.empty:
                ...
                logger.info(
                    f"Mean Predicted PnL: ${predictions['predicted_net_profit'].mean():.2f}"
                )
                logger.info(
                    f"Average Confidence: {predictions['prediction_confidence'].mean():.1f}%"
                )

                if "drift_detected" in predictions.columns:
                    drift_count = predictions["drift_detected"].sum()
                    logger.info(
                        f"Drift Detected: {drift_count} accounts ({drift_count / len(predictions) * 100:.1f}%)"
                    )

                if "is_high_risk" in predictions.columns:
                    high_risk_count = predictions["is_high_risk"].sum()
                    logger.info(
                        f"High Risk: {high_risk_count} accounts ({high_risk_count / len(predictions) * 100:.1f}%)"
                    )
                ...
                if shadow_cols:
                    ...
                    for col in shadow_cols:
                        model_name = col.replace("shadow_", "").replace(
                            "_prediction", ""
                        )
