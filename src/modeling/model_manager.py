...
class ModelVersionManager:
    ...
    def list_models(
        self, limit: int = 20, include_inactive: bool = True
    ) -> pd.DataFrame:
    ...
    def activate_model(
        self, model_version: str, deactivate_others: bool = True
    ) -> Dict[str, Any]:
        """Activate a specific model version."""
        # Verify model exists
        check_query = (
            "SELECT model_version FROM model_registry WHERE model_version = %s"
        )
        ...
        try:
            ...
            if deactivate_others:
                ...
                try:
                    deactivate_query = (
                        "UPDATE model_registry_enhanced SET is_active = false"
                    )
                    self.db_manager.model_db.execute_command(deactivate_query)
                except Exception:
            ...
            activate_query = (
                "UPDATE model_registry SET is_active = true WHERE model_version = %s"
            )
    ...
    def deactivate_model(self, model_version: str) -> Dict[str, Any]:
        ...
        try:
            ...
            deactivate_query = (
                "UPDATE model_registry SET is_active = false WHERE model_version = %s"
            )
            ...
            try:
                ...
                self.db_manager.model_db.execute_command(
                    deactivate_query, (model_version,)
                )
            except Exception:
                pass
    ...
    def compare_models(self, model_versions: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            comparison_df = self.db_manager.model_db.execute_query_df(
                query, tuple(model_versions)
            )
        ...
        if "test_mae" in comparison_df.columns:
            ...
            comparison_df["mae_relative_to_best"] = (
                comparison_df["test_mae"] / best_mae - 1
            ) * 100

        if "test_r2" in comparison_df.columns:
            best_r2 = comparison_df["test_r2"].max()
            comparison_df["r2_relative_to_best"] = (
                comparison_df["test_r2"] / best_r2 - 1
            ) * 100

        return comparison_df

    def get_model_performance_over_time(
        self, model_version: str, days_back: int = 30
    ) -> pd.DataFrame:
        ...
        try:
            ...
            performance_df["mae_rolling_7d"] = (
                performance_df["mae"].rolling(window=7, min_periods=1).mean()
            )
            performance_df["direction_accuracy_rolling_7d"] = (
                performance_df["direction_accuracy"]
    ...
    def cleanup_old_models(
        self, keep_latest: int = 10, keep_active: bool = True, dry_run: bool = True
    ) -> Dict[str, Any]:
        ...
        if not models_to_remove:
            return {"status": "no_action", "message": "No models eligible for cleanup"}
        ...
        if not dry_run:
            ...
            for model in models_to_remove:
                try:
                    ...
                    self.db_manager.model_db.execute_command(
                        delete_query, (model["model_version"],)
                    )
                    ...
                    logger.error(
                        f"Failed to remove model {model['model_version']}: {e}"
                    )
        else:
            ...
            if file_path and file_path != "None":
                ...
                if not path_obj.exists():
                    validation_results["errors"].append(
                        f"Missing {artifact_name}: {file_path}"
                    )
...
def main():
    ...
    list_parser.add_argument(
        "--limit", type=int, default=20, help="Maximum number of models to show"
    )
    list_parser.add_argument(
        "--include-inactive", action="store_true", help="Include inactive models"
    )
    ...
    activate_parser.add_argument(
        "--keep-others", action="store_true", help="Keep other models active"
    )
    ...
    compare_parser.add_argument(
        "model_versions", nargs="+", help="Model versions to compare"
    )

    # Performance over time
    performance_parser = subparsers.add_parser(
        "performance", help="Show model performance over time"
    )
    ...
    cleanup_parser.add_argument(
        "--keep-latest", type=int, default=10, help="Number of latest models to keep"
    )
    cleanup_parser.add_argument(
        "--remove-active", action="store_true", help="Allow removing active models"
    )
    cleanup_parser.add_argument(
        "--execute",
        action="store_true",
        ...
        if args.command == "list":
            ...
            if not models_df.empty:
                ...
                for _, model in models_df.iterrows():
                    active_status = (
                        "ACTIVE" if model.get("is_active", False) else "inactive"
                    )
                    print(
                        f"{model['model_version']:20} | {active_status:8} | "
            ...
            result = manager.activate_model(
                args.model_version, deactivate_others=not args.keep_others
            )
            ...
            performance_df = manager.get_model_performance_over_time(
                args.model_version, days_back=args.days
            )
            ...
            if not performance_df.empty:
                ...
                print(
                    f"Average Direction Accuracy: {performance_df['direction_accuracy'].mean():.1%}"
                )
                print(
                    f"Total Accounts Evaluated: {performance_df['accounts_evaluated'].sum()}"
                )
            else:
