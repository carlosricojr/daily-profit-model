...
class ConservativeConfidenceEstimator(BaseConfidenceEstimator):
    ...
    def fit(
        self, y_true: np.ndarray, y_pred: np.ndarray, X: Optional[pd.DataFrame] = None
    ):
        ...
        logger.info(
            f"Fitted conservative CI estimator: mean_error={self.error_stats['mean']:.2f}, "
    ...
    def estimate_intervals(
        ...
        if hasattr(self, "y_pred_train"):
            ...
            scale_factors = 1 + 0.1 * np.abs(
                (predictions - pred_mean) / (pred_std + 1e-6)
            )
...
class BalancedConfidenceEstimator(BaseConfidenceEstimator):
    ...
    def get_calibrated_intervals(
        ...
        if calibration_data is not None:
            ...
            logger.info(
                f"Calibrated intervals: actual coverage={coverage:.3f}, target={self.coverage:.3f}"
            )
...
class AggressiveConfidenceEstimator(BaseConfidenceEstimator):
    ...
    def fit(
        self, y_true: np.ndarray, y_pred: np.ndarray, X: pd.DataFrame, base_model=None
    ):
        ...
        logger.info(
            f"Fitted ensemble with {len(self.bootstrap_models)} bootstrap models"
        )
        ...
        for i in range(self.n_bootstrap):
            ...
            if base_model is not None:
                ...
                model = lgb.LGBMRegressor(
                    n_estimators=50, random_state=42 + i, verbosity=-1
                )
    ...
    def _get_bootstrap_intervals(
        ...
        bootstrap_preds = np.array(
            [model.predict(X) for model in self.bootstrap_models]
        )
    ...
    def get_uncertainty_decomposition(
        ...
        if self.bootstrap_models:
            ...
            aleatoric = np.mean(
                [np.abs(pred - predictions) for pred in bootstrap_preds], axis=0
            )
    ...
    def plot_coverage_analysis(
        ...
        ax.scatter(
            range(len(y_pred)), y_pred[sorted_idx], alpha=0.6, label="Predictions"
        )
        ax.fill_between(
            range(len(y_pred)),
            lower[sorted_idx],
            upper[sorted_idx],
            alpha=0.3,
            label=f"{self.coverage * 100:.0f}% CI",
        )
        ax.scatter(
            range(len(y_true)),
            y_true[sorted_idx],
