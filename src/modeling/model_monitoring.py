...
class ConservativeMonitor(BaseMonitor):
    ...
    def monitor_predictions(
        self, predictions_df: pd.DataFrame, actuals_df: Optional[pd.DataFrame] = None
    ):
        ...
        if actuals_df is not None:
            ...
            if self._check_performance_degradation(metrics):
                logger.warning(
                    f"Performance degradation detected for model {self.model_version}"
                )
    ...
    def _check_prediction_anomalies(self, predictions_df: pd.DataFrame):
        ...
        if np.isnan(predictions).any() or np.isinf(predictions).any():
            ...
            self._send_alert(
                "invalid_predictions", {"count": np.isnan(predictions).sum()}
            )
...
class BalancedMonitor(ConservativeMonitor):
        ...
        for column in current_features.columns:
            if column in reference_features.columns:
                ...
                if ks_stat > self.drift_threshold:
                    logger.warning(
                        f"Feature drift detected in {column}: KS={ks_stat:.3f}"
                    )

        return drift_scores

    def monitor_prediction_drift(
        self, current_predictions: np.ndarray, reference_predictions: np.ndarray
    ) -> Dict[str, float]:
        ...
        if drift_metrics["is_drifted"]:
            logger.warning(
                f"Prediction drift detected: KS={ks_stat:.3f}, PSI={psi:.3f}"
            )
    ...
    def create_monitoring_dashboard_data(self) -> Dict[str, Any]:
        ...
        recent_metrics = self.db_manager.model_db.execute_query_df(
            query, (self.model_version,)
        )
...
class AggressiveMonitor(BalancedMonitor):
    ...
    def _check_performance_health(self) -> Dict[str, Any]:
        ...
        metrics_df = self.db_manager.model_db.execute_query_df(
            query, (self.model_version,)
        )
    ...
    def _check_prediction_health(self) -> Dict[str, Any]:
        ...
        predictions_df = self.db_manager.model_db.execute_query_df(
            query, (self.model_version,)
        )
        ...
        extreme_ratio = (
            np.abs(predictions) > np.percentile(np.abs(predictions), 99)
        ).mean()
    ...
    def trigger_automated_retraining(self, reason: str) -> bool:
        ...
        self.db_manager.model_db.execute_command(
            query, (self.model_version, reason, datetime.now(), "triggered")
        )
...
class ExperimentTracker:
    ...
    def create_experiment(
        ...
        logger.info(
            f"Created experiment {experiment_id}: {control_model} vs {treatment_model}"
        )
    ...
    def assign_to_variant(self, account_id: str, experiment_id: str) -> str:
        ...
        hash_value = int(
            hashlib.md5(f"{account_id}_{experiment_id}".encode()).hexdigest()[:8], 16
        )
        assignment = (
            "treatment" if (hash_value % 100) < (traffic_split * 100) else "control"
        )
        ...
        self.db_manager.model_db.execute_command(
            query, (experiment_id, account_id, assignment, datetime.now())
        )
    ...
    def analyze_experiment(self, experiment_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "direction_accuracy_improvement": treatment["direction_accuracy"]
            - control["direction_accuracy"],
            "t_statistic": t_stat,
            "p_value": p_value,
...
class PerformanceAnalyzer:
    ...
    def analyze_performance_by_segment(
        self, predictions_df: pd.DataFrame, actuals_df: pd.DataFrame
    ) -> Dict[str, Any]:
        """Analyze performance across different segments."""
        merged_df = predictions_df.merge(actuals_df, on=["login", "prediction_date"])
        merged_df["error"] = (
            merged_df["predicted_net_profit"] - merged_df["actual_net_profit"]
        )
        ...
        for segment in merged_df["pred_magnitude"].unique():
            ...
            segments[f"magnitude_{segment}"] = {
                ...
                "direction_accuracy": (
                    (segment_data["predicted_net_profit"] > 0)
                    == (segment_data["actual_net_profit"] > 0)
                ).mean(),
            }

        # By day of week
        merged_df["day_of_week"] = pd.to_datetime(
            merged_df["prediction_date"]
        ).dt.dayofweek
        for day in range(7):
    ...
    def identify_failure_patterns(
        ...
        account_failures = merged_df.groupby("login")["is_failure"].agg(
            ["sum", "count"]
        )
        account_failures["failure_rate"] = (
            account_failures["sum"] / account_failures["count"]
        )
