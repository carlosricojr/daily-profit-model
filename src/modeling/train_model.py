...
class ModelMonitor:
    ...
    def check_model_degradation(
        self, model_version: str, current_metrics: Dict[str, float]
    ) -> Dict[str, Any]:
        ...
        try:
            historical_results = self.db_manager.model_db.execute_query(
                query, (model_version,)
            )
            ...
            if not historical_results:
                return {
                    ...
                    "message": "No historical models for comparison",
                }
            ...
            mae_degradation = (
                (current_mae - historical_mae) / historical_mae
                if historical_mae > 0
                else 0
            )
            r2_degradation = (
                (historical_r2 - current_r2) / abs(historical_r2)
                if historical_r2 != 0
                else 0
            )
...
class EnhancedModelTrainer:
    ...
    def __init__(self, model_version: Optional[str] = None):
        ...
        self.model_version = (
            model_version or f"v1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def train_model(
        ...
        n_trials: int = 50,
        enable_prediction_intervals: bool = True,
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Starting enhanced model training - Version: {self.model_version}"
            )
            ...
            metrics["improvement_over_baseline"] = (
                (baseline_metrics["test_mae"] - metrics["test_mae"])
            ...
            degradation_check = self.monitor.check_model_degradation(
                self.model_version, metrics
            )
            ...
            results = {
                ...
                "data_splits": {
                    "train": {
                        "start": splits["dates_train"].min(),
                        "end": splits["dates_train"].max(),
                        "samples": len(splits["X_train"]),
                    },
                    "val": {
                        "start": splits["dates_val"].min(),
                        "end": splits["dates_val"].max(),
                        "samples": len(splits["X_val"]),
                    },
                    "test": {
                        "start": splits["dates_test"].min(),
                        "end": splits["dates_test"].max(),
                        "samples": len(splits["X_test"]),
                    },
                },
            }

            logger.info(
                f"Enhanced model training completed - Version: {self.model_version}"
            )
            return results
    ...
    def _load_training_data(self) -> Tuple[pd.DataFrame, pd.Series, pd.Series]:
        ...
        logger.info(
            f"Date range: {df['prediction_date'].min()} to {df['prediction_date'].max()}"
        )
        ...
        if missing_features.any():
            logger.warning(
                f"Features with missing values: {missing_features[missing_features > 0].to_dict()}"
            )
    ...
    def _tune_hyperparameters(
        ...
        def objective(trial):
            params = {
                "objective": trial.suggest_categorical(
                    "objective", ["regression_l1", "huber", "fair"]
                ),
                "metric": "mae",
                "boosting_type": "gbdt",
        ...
        study = optuna.create_study(
            direction="minimize", sampler=TPESampler(seed=42, n_startup_trials=10)
        )
        study.optimize(objective, n_trials=n_trials)

        logger.info(
            f"Hyperparameter optimization completed: Best MAE = {study.best_value:.4f}"
        )
    ...
    def _calculate_shap_values(self, X_sample: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            shap.summary_plot(
                shap_values, X_sample, feature_names=self.feature_columns, show=False
            )
            plt.savefig(
                self.artifacts_dir / "shap_summary.png", dpi=150, bbox_inches="tight"
            )
            plt.close()
    ...
    def _save_enhanced_artifacts(
        ...
        if self.prediction_intervals.lower_model is not None:
            joblib.dump(
                ...
                self.artifacts_dir / "prediction_intervals.pkl",
            )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"  Direction Accuracy: {results['metrics'].get('test_direction_accuracy', 0):.1%}"
        )
        logger.info(f"  Baseline MAE: ${results['baseline_metrics']['test_mae']:.2f}")
        logger.info(
            f"  Improvement: {results['metrics']['improvement_over_baseline']:.1f}%"
        )
        ...
        if results["degradation_check"]["status"] == "degraded":
            logger.warning(
                f"  Alerts: {', '.join(results['degradation_check']['alerts'])}"
            )
