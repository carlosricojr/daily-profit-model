...
class RegimesIngester(BaseIngester):
    ...
    def _validate_record(self, record: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        json_fields = ['market_news', 'instruments', 'country_economic_indicators', 
                      'news_analysis', 'summary']
        
        for field in json_fields:
            value = record.get(field)
            if value is not None and not isinstance(value, (dict, list, str, type(None))):
    ...
    def ingest_regimes(self,
        ...
        try:
            ...
            if checkpoint and checkpoint.get('last_processed_date'):
                checkpoint_date = datetime.strptime(checkpoint['last_processed_date'], '%Y-%m-%d').date()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Ingest regimes data with enhanced features')
