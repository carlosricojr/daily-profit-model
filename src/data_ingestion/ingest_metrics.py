...
class MetricsIngester(BaseIngester):
    ...
    def __init__(self, 
        ...
        for metric_type in MetricType:
            ...
            self.checkpoint_managers[metric_type.value] = CheckpointManager(
                checkpoint_file, 
                f"metrics_{metric_type.value}"
            )
            self.metrics_by_type[metric_type.value] = IngestionMetrics()
    
    def _validate_record(self, record: Dict[str, Any], metric_type: MetricType) -> Tuple[bool, List[str]]:
        ...
        numeric_fields = [
            'netProfit', 'grossProfit', 'grossLoss', 'totalTrades',
        ...
        if (total_trades is not None and winning_trades is not None and 
            losing_trades is not None):
            if winning_trades + losing_trades > total_trades:
                errors.append('inconsistent_trade_counts')
        
        return len(errors) == 0, errors
    
    def _get_record_key(self, record: Dict[str, Any], metric_type: MetricType) -> Optional[str]:
    ...
    def _transform_alltime_metric(self, metric: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'gross_profit': self._safe_float(metric.get('grossProfit')),
            'gross_loss': self._safe_float(metric.get('grossLoss')),
            'total_trades': self._safe_int(metric.get('totalTrades')),
            'winning_trades': self._safe_int(metric.get('winningTrades')),
            'losing_trades': self._safe_int(metric.get('losingTrades')),
            'win_rate': self._safe_float(metric.get('winRate')),
            'profit_factor': self._safe_float(metric.get('profitFactor')),
            'average_win': self._safe_float(metric.get('averageWin')),
            'average_loss': self._safe_float(metric.get('averageLoss')),
            'average_rrr': self._safe_float(metric.get('averageRRR')),
            'expectancy': self._safe_float(metric.get('expectancy')),
            'sharpe_ratio': self._safe_float(metric.get('sharpeRatio')),
            'sortino_ratio': self._safe_float(metric.get('sortinoRatio')),
            'max_drawdown': self._safe_float(metric.get('maxDrawdown')),
            'max_drawdown_pct': self._safe_float(metric.get('maxDrawdownPct')),
    ...
    def _transform_daily_metric(self, metric: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'win_rate': self._safe_float(metric.get('winRate')),
            'profit_factor': self._safe_float(metric.get('profitFactor')),
            'lots_traded': self._safe_float(metric.get('lotsTraded')),
            'volume_traded': self._safe_float(metric.get('volumeTraded')),
            'commission': self._safe_float(metric.get('commission')),
            'swap': self._safe_float(metric.get('swap')),
            'balance_start': self._safe_float(metric.get('balanceStart')),
            'balance_end': self._safe_float(metric.get('balanceEnd')),
            'equity_start': self._safe_float(metric.get('equityStart')),
            'equity_end': self._safe_float(metric.get('equityEnd')),
            'ingestion_timestamp': datetime.now(),
    ...
    def _get_conflict_clause(self, table_name: str) -> str:
        ...
        if 'alltime' in table_name:
            ...
            return "ON CONFLICT (account_id, date, hour, ingestion_timestamp) DO NOTHING"
        ...
        try:
            ...
            if metric_type_enum == MetricType.ALLTIME:
                ...
                total_records = self._ingest_time_series_metrics(
                    metric_type_enum, start_date, end_date, 
                    logins, accountids, checkpoint
                )
            ...
            for page_num, metrics_page in enumerate(self.api_client.get_metrics(
                ...
                for metric in metrics_page:
                    ...
                    if self.enable_validation:
                        is_valid, errors = self._validate_record(metric, MetricType.ALLTIME)
                        if not is_valid:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Ingest metrics data with enhanced features')
