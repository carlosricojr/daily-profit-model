...
class PlansIngester(BaseIngester):
    ...
    COLUMN_MAPPING = {
        # Plan identification
        'plan_id': ['plan_id', 'plan id', 'planid', 'plan_code', 'plan code'],
        'plan_name': ['plan_name', 'plan name', 'planname', 'name'],
        'plan_type': ['plan_type', 'plan type', 'type', 'plantype'],
        
        # Financial parameters
        'starting_balance': ['starting_balance', 'starting balance', 'startingbalance', 'initial_balance', 'balance'],
        'profit_target': ['profit_target', 'profit target', 'profittarget', 'target'],
        'profit_target_pct': ['profit_target_pct', 'profit target %', 'profit_target_%', 'profit_target_percent'],
        'max_drawdown': ['max_drawdown', 'max drawdown', 'maxdrawdown'],
        'max_drawdown_pct': ['max_drawdown_pct', 'max drawdown %', 'max_drawdown_%', 'max_drawdown_percent'],
        'max_daily_drawdown': ['max_daily_drawdown', 'max daily drawdown', 'maxdailydrawdown', 'daily_drawdown'],
        'max_daily_drawdown_pct': ['max_daily_drawdown_pct', 'max daily drawdown %', 'max_daily_drawdown_%', 'daily_drawdown_pct'],
        
        # Trading parameters
        'max_leverage': ['max_leverage', 'max leverage', 'maxleverage', 'leverage'],
        'is_drawdown_relative': ['is_drawdown_relative', 'is drawdown relative', 'drawdown_relative', 'relative_drawdown'],
        'min_trading_days': ['min_trading_days', 'min trading days', 'mintradingdays', 'minimum_days'],
        'max_trading_days': ['max_trading_days', 'max trading days', 'maxtradingdays', 'maximum_days'],
        'profit_split_pct': ['profit_split_pct', 'profit split %', 'profit_split_%', 'profit_split', 'split']
    }
    ...
    def _map_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for target_col, possible_names in self.COLUMN_MAPPING.items():
            ...
            normalized_possible = [self._normalize_column_name(name) for name in possible_names]
    ...
    def _validate_record(self, record: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if plan_id is None or (isinstance(plan_id, str) and plan_id.strip() == '') or pd.isna(plan_id):
        ...
        if starting_balance_raw is not None and starting_balance_raw != '' and pd.notna(starting_balance_raw):
        ...
        percentage_fields = [
            ...
            'max_daily_drawdown_pct', 'profit_split_pct'
        ]
    ...
    def _transform_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Convert percentages
        for field in ['profit_target_pct', 'max_drawdown_pct', 
                     'max_daily_drawdown_pct', 'profit_split_pct']:
            if field in record:
                record[field] = self._convert_percentage(record[field])
        
        # Convert boolean
        if 'is_drawdown_relative' in record:
            record['is_drawdown_relative'] = self._convert_boolean(record['is_drawdown_relative'])
        ...
        return {
            ...
            'profit_target_pct': record.get('profit_target_pct'),
            ...
            'max_daily_drawdown_pct': record.get('max_daily_drawdown_pct'),
            'max_leverage': self._safe_int(record.get('max_leverage')),
            'is_drawdown_relative': record.get('is_drawdown_relative'),
            'min_trading_days': self._safe_int(record.get('min_trading_days')),
            'max_trading_days': self._safe_int(record.get('max_trading_days')),
            'profit_split_pct': record.get('profit_split_pct'),
    ...
    def _process_csv_file(self, file_path: Path, checkpoint: Optional[Dict[str, Any]] = None) -> int:
    ...
    def ingest_plans(self,
                    ...
                    enable_deduplication: Optional[bool] = None) -> int:
        """
        ...
        try:
            ...
            if specific_files:
                ...
                raise ValueError("Either csv_directory or specific_files must be provided")
