...
class RetryManager:
    ...
    def retry_operation(
        ...
        for attempt in range(config.max_attempts):
            ...
            try:
                logger.info(
                    f"Attempting {operation_name} (attempt {attempt + 1}/{config.max_attempts})"
                )
    ...
    def _calculate_stats(self, operation_name: str, history: list) -> Dict[str, Any]:
        ...
        successful_executions = sum(
            1 for h in history if h["final_status"] == "success"
        )
        ...
        return {
            ...
            "average_attempts": total_attempts / total_executions
            if total_executions > 0
            else 0.0,
