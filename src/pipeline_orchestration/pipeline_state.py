...
class PipelineState:
    ...
    def update_stage_checkpoint(
        self, stage_name: str, checkpoint_name: str, checkpoint_data: Dict[str, Any]
    ):
        """
        ...
        if stage_name in self.current_state["stage_states"]:
            self.current_state["stage_states"][stage_name]["checkpoints"][
                checkpoint_name
            ] = {"timestamp": datetime.now().isoformat(), "data": checkpoint_data}
            self._save_state()
    ...
    def complete_stage(
        ...
        if stage_name in self.current_state["stage_states"]:
            ...
            self.current_state["stage_states"][stage_name]["end_time"] = (
                datetime.now().isoformat()
            )

            if records_processed is not None:
                self.current_state["stage_states"][stage_name]["records_processed"] = (
                    records_processed
                )
    ...
    def get_stage_checkpoint(
        self, stage_name: str, checkpoint_name: str
        ...
        """
        if stage_name in self.current_state["stage_states"]:
            checkpoints = self.current_state["stage_states"][stage_name].get(
                "checkpoints", {}
            )
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        if self.start_time:
            end_time = datetime.fromisoformat(
                self.current_state.get("end_time", datetime.now().isoformat())
            )
