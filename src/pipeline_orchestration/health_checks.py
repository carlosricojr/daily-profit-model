...
class HealthChecker:
    ...
    def _analyze_trends(self) -> Dict[str, Any]:
        ...
        older_history = (
            self.health_history[-48:-24] if len(self.health_history) >= 48 else []
        )
        ...
        recent_failures = sum(
            1 for h in recent_history if h["overall_status"] in ["unhealthy", "error"]
        )
        trends["failure_rate_24h"] = (
            recent_failures / len(recent_history) if recent_history else 0
        )

        # Calculate average SLA violations
        recent_violations = [h["sla_violations"] for h in recent_history]
        trends["average_sla_violations"] = (
            sum(recent_violations) / len(recent_violations) if recent_violations else 0
        )
        ...
        if older_history:
            older_failures = sum(
                ...
                if h["overall_status"] in ["unhealthy", "error"]
            )
        ...
        total_score = sum(
            status_weights.get(h["overall_status"], 0) for h in recent_history
        )
        trends["stability_score"] = (
            total_score / len(recent_history) if recent_history else 0
        )
    ...
    def check_database_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            start_time = time.time()
            self.db_manager.source_db.execute_query(
                "SELECT 1 as test, current_timestamp as ts"
            )
                ...
                try:
                    ...
                    if query_info["name"] == "active_connections":
                        metrics["active_connections"] = (
                            result[0]["active_connections"] if result else 0
                        )
                    elif query_info["name"] == "slow_queries":
                        metrics["slow_queries"] = (
                            result[0]["slow_queries"] if result else 0
                        )

                except Exception as e:
                    logger.warning(
                        f"Performance query {query_info['name']} failed: {e}"
                    )
        ...
        try:
            ...
            for path, disk_data in disk_info.items():
                if disk_data["percent_used"] > 90:
                    status = "unhealthy"
                    issues.append(
                        f"Critical disk usage on {path}: {disk_data['percent_used']:.1f}%"
                    )
                elif disk_data["percent_used"] > 80:
                    if status == "healthy":
                        status = "warning"
                    issues.append(
                        f"High disk usage on {path}: {disk_data['percent_used']:.1f}%"
                    )
        ...
        try:
            ...
            for check in freshness_checks:
                try:
                    ...
                    results[check["name"]] = {"status": "error", "message": str(e)}
                    overall_status = "unhealthy"
                ...
                try:
                    ...
                    if result:
                        ...
                        if (
                            ...
                            and value != check["threshold"]
                        ):
                            status = "warning"
                            if value > check["threshold"] * 5:
                                status = "unhealthy"
                        elif (
                            check["comparison"] == "less_than"
                            and value >= check["threshold"]
                        ):
                            status = "warning"
    ...
    def check_pipeline_sla(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in results:
                ...
                duration = (
                    float(row["duration_minutes"]) if row["duration_minutes"] else 0
                )
                status = row["status"]
            ...
            if avg_duration > 360:  # 6 hours
                status = "unhealthy"
                issues.append(
                    f"Average duration exceeds SLA: {avg_duration:.1f} minutes"
                )
            elif avg_duration > 240:  # 4 hours
                status = "warning"
                issues.append(
                    f"Average duration approaching SLA limit: {avg_duration:.1f} minutes"
                )
    ...
    def check_model_availability(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pred_result = self.db_manager.model_db.execute_query(
                pred_query, (model["model_version"],)
            )
            recent_predictions = (
                pred_result[0]["recent_predictions"] if pred_result else 0
            )
            ...
            message = (
                f"Active model: {model['model_version']} (age: {model_age_days} days)"
            )
            if issues:
    ...
    def check_network_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_connection_time = sum(
                r["connection_time_ms"] for r in results if r["success"]
...
def run_health_check(
    verbose: bool = True, include_trends: bool = True
) -> Dict[str, Any]:
    """
    ...
    if verbose:
        ...
        summary = results["summary"]
        logger.info(
            f"Check Summary: {summary['healthy']} healthy, {summary['warnings']} warnings, "
        ...
        if results["sla_violations"]:
            ...
            for violation in results["sla_violations"]:
                logger.warning(
                    f"  {violation['sla_name']}: {violation['actual_value']} {violation['unit']} "
        ...
        for check_name, check_result in results["checks"].items():
            ...
            logger.info(
                f"{status_symbol} {check_name.upper()}: {check_result['status']}"
            )
