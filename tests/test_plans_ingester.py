...
class TestPlansIngester(unittest.TestCase):
    ...
    def test_boolean_conversion(self):
        ...
        for input_val, expected in test_cases:
            data = [{"plan_id": "PLAN123", "is_drawdown_relative": input_val}]
    ...
    def test_invalid_csv_handling(self):
        ...
        result = self.ingester.ingest_plans(
            csv_directory=str(self.csv_dir), enable_validation=True
        )
        ...
        self.assertEqual(
            self.ingester.metrics.invalid_records, 3
        )  # PLAN2, PLAN3, PLAN4
    ...
    def test_specific_files_processing(self):
        ...
        self.create_test_csv(
            "plans_old.csv", [{"plan_id": "OLD1", "starting_balance": 1000}]
        )
        target_file = self.create_test_csv(
            "plans_new.csv", [{"plan_id": "NEW1", "starting_balance": 2000}]
        )
        self.create_test_csv(
            "plans_temp.csv", [{"plan_id": "TEMP1", "starting_balance": 3000}]
        )
    ...
    def test_checkpoint_resume(self):
        ...
        data = [
            {"plan_id": f"PLAN{i}", "starting_balance": i * 1000} for i in range(100)
        ]
        ...
        # Process with resume
        result = self.ingester.ingest_plans(
            csv_directory=str(self.csv_dir), resume_from_checkpoint=True
        )
