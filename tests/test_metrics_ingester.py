...
class TestMetricsIngester(unittest.TestCase):
    ...
    def test_validate_record(self):
        ...
        is_valid, errors = self.ingester._validate_record(valid_alltime, MetricType.ALLTIME)
        ...
        is_valid, errors = self.ingester._validate_record(invalid_metric, MetricType.ALLTIME)
        ...
        is_valid, errors = self.ingester._validate_record(invalid_trades, MetricType.ALLTIME)
        self.assertFalse(is_valid)
    ...
    def test_validate_daily_metric(self):
        ...
        is_valid, errors = self.ingester._validate_record(invalid_date, MetricType.DAILY)
        self.assertFalse(is_valid)
    ...
    def test_validate_hourly_metric(self):
        ...
        is_valid, errors = self.ingester._validate_record(valid_hourly, MetricType.HOURLY)
        self.assertTrue(is_valid)
        self.assertEqual(len(errors), 0)
        ...
        is_valid, errors = self.ingester._validate_record(invalid_hour, MetricType.HOURLY)
        self.assertFalse(is_valid)
    ...
    def test_metrics_tracking(self):
        ...
        self.mock_api_client.get_metrics.return_value = iter([
            [
                ...
                {'accountId': 'ACC2', 'login': 2, 'netProfit': 'invalid'},  # Invalid
    ...
    def test_date_range_processing(self):
        ...
        for day in range(5):
            api_responses.append([
                {'accountId': f'ACC{i}', 'login': i, 'date': f'2024011{day}', 'netProfit': i * 10}
                for i in range(1, 4)
            ])
        
        self.mock_api_client.get_metrics.side_effect = [iter([resp]) for resp in api_responses]
        self.mock_api_client.format_date.side_effect = lambda d: d.strftime('%Y%m%d')
    ...
    def test_api_error_handling(self):
        ...
        self.mock_api_client.get_metrics.side_effect = APIError("API rate limit exceeded")
